...
angular.module('dialogs.controllers', ['ui.bootstrap.modal'])
         ...
         */
        .controller('errorDialogCtrl', ['$scope', '$modalInstance', 'header', 'msg', 'closeButtonLabel', function($scope, $modalInstance, header, msg, closeButtonLabel) {
                //-- Variables -----//

                $scope.header = (angular.isDefined(header)) ? header : 'Error';
                $scope.msg = (angular.isDefined(msg)) ? msg : 'An unknown error has occurred.';
                $scope.closeButtonLabel = (angular.isDefined(closeButtonLabel)) ? closeButtonLabel : 'Ok';
         ...
         */
        .controller('notifyDialogCtrl', ['$scope', '$modalInstance', 'header', 'msg', 'closeButtonLabel', function($scope, $modalInstance, header, msg, closeButtonLabel) {
                //-- Variables -----//
         ...
         */
        .controller('confirmDialogCtrl', ['$scope', '$modalInstance', 'header', 'msg', 'confirmButtonLabel', function($scope, $modalInstance, header, msg, confirmButtonLabel) {
                //-- Variables -----//

                $scope.header = (angular.isDefined(header)) ? header : 'Confirmation';
                $scope.msg = (angular.isDefined(msg)) ? msg : 'Confirmation required.';
                $scope.confirmButtonLabel = (angular.isDefined(confirmButtonLabel)) ? confirmButtonLabel : 'Yes';
                //-- Methods -----//
                            ...
                            return $modal.open({
                                ...
                                resolve: {
                                    closeButtonLabel: function() {
                                        return angular.copy(that.closeButtonLabel);
                                    },
                                    header: function() {
                            ...
                            return $modal.open({
                                ...
                                resolve: {
                                    confirmButtonLabel: function() {
                                        return angular.copy(that.confirmButtonLabel);
                                    },
                                    header: function() {
                                        return angular.copy(header);
                                    },
...
angular.module('dialogs', ['dialogs.services', 'ngSanitize']) // requires angular-sanitize.min.js (ngSanitize) //code.angularjs.org/1.2.1/angular-sanitize.min.js
        ...
        .run(['$templateCache', function($templateCache) {
                $templateCache.put('/dialogs/error.html', '<div class="modal-header dialog-header-error"><button type="button" class="close" ng-click="close()">&times;</button><h4 class="modal-title text-danger"><span class="glyphicon glyphicon-warning-sign"></span> <span ng-bind-html="header"></span></h4></div><div class="modal-body text-danger" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" class="btn btn-default" ng-click="close()">{{closeButtonLabel}}</button></div>');
                ...
                $templateCache.put('/dialogs/confirm.html', '<div class="modal-header dialog-header-confirm"><button type="button" class="close" ng-click="no()">&times;</button><h4 class="modal-title"><span class="glyphicon glyphicon-check"></span> {{header}}</h4></div><div class="modal-body" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" class="btn btn-default" ng-click="yes()">{{confirmButtonLabel}}</button><button type="button" class="btn btn-primary" ng-click="no()">No</button></div>');
            }]); // end run / dialogs


>>>>>>> Trasformed the '$dialogs' service in a provider
